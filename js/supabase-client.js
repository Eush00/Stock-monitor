...
class SupabaseClient {
  ...
  async upsertFundamentals(fundamentals) {
    try {
      ...
      console.log(
        `🏢 Aggiornamento fundamentals per ${fundamentals.symbol}...`
      );
  ...
  async logApiCall(
    ...
    try {
      ...
      const logData = {
        ...
        reset_time: new Date(
          Date.now() + CONFIG.rateLimits[provider]?.windowMs || 3600000
        ),
      };

      const { error } = await this.client.from('api_calls').insert(logData);
